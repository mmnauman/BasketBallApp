...
function FirstQuarter() {
  ...
  function changeHandler1(evnt) {
    if (!states.includes(evnt)) {
      ...
      console.log("error");
      setErrorContent(
        ` You have already selected this Player ${evnt.firstName} ${evnt.lastName}. You cannot select same player Again `
      );
      setShowError(true);
    }
    console.log(evnt);
  ...
  function SelectHandler1(evnt) {
    if (!selectedStates.includes(evnt)) {
      ...
      evnt.disabled = false;
      setErrorContent(
        `You Have Already Selected This Position ${evnt.position}. You Cannot Select Same Position Again `
      );
      setShowError(true);
    }
    console.log(evnt);
  ...
  return isLoaded === false ? (
    ...
    <TeamsContainerStyled>
      ...
      {ShowError && (
        <ErrorComp closeHandler={closeHandler}>{ErrorContent}</ErrorComp>
      )}
